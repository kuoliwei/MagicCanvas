...
public class ScratchCard : MonoBehaviour, IPointerDownHandler
{
    ...
    private void Update()
    {
        ...
        //if (timer >= 1f)
        //{
        //    Debug.Log($"[ScratchCardTest] 每秒刮除次數（透過 WebSocketMessageReceiver -> ReceiveMessage）：{scratchCount}");
        //    scratchCount = 0;
        //    timer = 0f;
        //}
        //{//一幀處裡一筆uv
        ...
        //        //brushMultiPos[0] = new Vector4(uv.x, uv.y, sizeX, sizeY); // 只設定第一筆
        //        //brushMultiPos[1] = new Vector4(1 - uv.x, 1 - uv.y, sizeX, sizeY); // 只設定第二筆
        //        //brushMultiPos[1] = new Vector4(uv.x, 1 - uv.y, sizeX, sizeY); // 只設定第二筆
        //        //brushMultiPos[1] = new Vector4(1 - uv.x, uv.y, sizeX, sizeY); // 只設定第二筆
        //        for (int i = 0; i < brushMultiPos.Length; i++)
        //        {
        //            brushMultiPos[i] = new Vector4(uv.x + 0.01f * i, uv.y + 0.01f * i, sizeX, sizeY); // 只設定第一筆
        //        }
        ...
        //        RenderTexture tempRT = RenderTexture.GetTemporary(renderTex.width, renderTex.height, 0, RenderTextureFormat.ARGB32);
        //        Graphics.Blit(renderTex, tempRT, eraseMaterial); // shader 輸出到 temp
        //        Graphics.Blit(tempRT, renderTex);                // 再複製回來
        //        RenderTexture.ReleaseTemporary(tempRT);
        ...
        //        Debug.Log($"[Test] 傳入 {uvList.Count} 筆 UV 至 DrawTransparentAt");
        //    }
        //}
